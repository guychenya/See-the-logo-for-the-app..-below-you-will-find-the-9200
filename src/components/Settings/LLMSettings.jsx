...
function LLMSettings() {
  ...
  const tabs = [
    ...
    { id: 'ollama-setup', label: 'Ollama Setup', icon: FiCheckCircle }
  ];
            ...
            <div className="flex items-center gap-3">
              ...
              {testResult && !testResult.testing && (
                <div className={`flex items-center gap-2 text-sm ${testResult.success ? 'text-green-400' : 'text-red-400'}`}>
                  <SafeIcon icon={testResult.success ? FiCheckCircle : FiAlertCircle} className="w-4 h-4" />
                  {testResult.message}
                </div>
              )}
            </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'ollama-setup' && <OllamaSetup />}
    </div>
  );
}
